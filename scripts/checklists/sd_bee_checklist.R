...
#   5. sd_holway_native_bee_checklist_generated.csv              <- Holway reference table -> lookup subtree
#   6. sd_inat_native_bee_checklist_generated.csv            <- RAW iNat bee obs clipped to the SD county
#        boundary -> lookup subtree (leaves + ancestor rows). NOT inat_bee_clean.R.
#   7. sd_holway_and_inat_native_bee_checklist_generated.csv <- union of #5 + #6, one row per taxon with
...
build_sd_bee_checklists <- function(bees_sf, lookup, holway_sub = NULL, holway_reference = NULL) {
  ...
  write_csv(sd_inat, file.path(SD_OUT_DIR, "sd_inat_native_bee_checklist_generated.csv"), na = "")

  # 5. Holway: the Holway subtree, in the shared format.
  if (!is.null(holway_sub))
    write_csv(holway_sub, file.path(SD_OUT_DIR, "sd_holway_native_bee_checklist_generated.csv"), na = "")
  ...
  write_csv(sd_combined, file.path(SD_OUT_DIR, "sd_holway_and_inat_native_bee_checklist_generated.csv"), na = "")
