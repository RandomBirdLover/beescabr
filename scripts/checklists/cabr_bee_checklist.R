...
#   1. cabr_inat_native_bee_checklist_generated.csv  <- RAW iNat bee obs clipped to the CABR survey box,
#        expanded to the lookup subtree (leaves + ancestor rows). NOT inat_bee_clean.R.
#   2. cabr_specimen_native_bee_checklist_generated.csv  <- the cleaned specimens (cabr_specimen_bee_clean_generated.csv),
#        expanded to the lookup subtree (specimen-only leaves kept with blank taxon_id).
#   3. cabr_official_native_bee_checklist_generated.csv  <- union of #1 + #2, one row per taxon, with boolean
...
# lookup    : the taxonomy lookup (sd_bee_taxonomy_lookup_generated.csv) -- the normalized tree source.
# specimens : cleaned specimen records (cabr_specimen_bee_clean_generated.csv) -- NULL if not built yet.
...
build_cabr_bee_checklists <- function(bees_sf, lookup, specimens = NULL, holway_sub = NULL) {
  ...
  write_csv(cabr_inat, file.path(CABR_OUT_DIR, "cabr_inat_native_bee_checklist_generated.csv"), na = "")
  ...
  if (!is.null(cabr_specimen))
    write_csv(cabr_specimen, file.path(CABR_OUT_DIR, "cabr_specimen_native_bee_checklist_generated.csv"), na = "")
  ...
  write_csv(cabr_official, file.path(CABR_OUT_DIR, "cabr_official_native_bee_checklist_generated.csv"), na = "")
