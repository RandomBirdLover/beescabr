...
rows_html <- vapply(seq_len(nrow(tbl)), function(i) {
  ...
  sprintf(paste0('<tr class="%s"><td class="bee"><i>%s</i>%s</td><td class="num">%d</td>',
                 '<td>%s</td><td>%s</td><td>%s</td><td><span class="pill %s">%s</span></td><td class="loc">%s</td></tr>'),
          if (low) "low" else "", esc(r$bee),
          if (has(r$common_name)) paste0('<span class="cn">', esc(r$common_name), '</span>') else "",
          r$n_records, esc(r$peak_day), esc(r$active_months), esc(r$top_flowers),
          diet_class(r$diet), esc(r$diet), esc(r$where_to_find))
}, character(1))
html <- paste0(
...
'<p class="sub">One row per bee species. Peak day = circular mean of record dates; active months = 5th-95th percentile; diet = number of plant genera used; where = favoured transect(s) or an off-transect centre + buffer. Rows in grey have &lt;10 records (peak/season are rough). Click a column header to sort.</p>',
'<table id="t"><thead><tr>',
'<th>Bee</th><th class="num">Records</th><th>Peak day</th><th>Active months</th><th>Top flowers</th><th>Diet</th><th>Where to find</th>',
...
if (requireNamespace("gridExtra", quietly = TRUE) && requireNamespace("ggplot2", quietly = TRUE)) {
  disp <- tbl %>% transmute(Bee = bee, N = n_records, `Peak day` = peak_day,
                            `Active months` = active_months, `Top flowers` = top_flowers,
                            Diet = diet, `Where to find` = where_to_find)
  ...
  ggplot2::ggsave(file.path(OUT_DIR, "bee_field_guide.png"), g,
                  width = 14, height = 0.24 * nrow(disp) + 1, dpi = 200, limitsize = FALSE, bg = "white")
