# =============================================================
# clean/survey_transects.R
# beescabr -- resolve each BEEPLE survey day to ONE transect (the MAJORITY of that
# surveyor's transect tags that day), then stamp it onto every one of their obs for
# that date. Fixes stray mis-tags: 50 TP + 3 UPMON -> the whole day reads TP.
#
#   * INTERNS are left alone -- they walk a multi-transect route. Only status=="keep"
...
if (!exists("BEESCABR_SOURCED_BY_RUNNER") && sys.nframe() == 0)
  message("Sourced survey_transects.R -- resolve_transects(membership) is called by the brain.")
