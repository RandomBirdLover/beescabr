# =============================================================
# Clean Non-Lethal iNaturalist Plant Data (Intern + Beeple)
# beescabr pipeline
# Author: Brandi Sanchez  |  Written: 2026-07-08
#
# Parallel to inat_bee_clean.R -- same logic, same crosswalk,
# but scoped to Plantae observations instead of bees.
#
...
#   1. FETCH   plant observations from the iNaturalist API (v1).
#              Scope = roster observers, CABR box, Plantae (taxon 47126),
#              any grade.
...
#   4. TRIAGE  every observation against master_crosswalk.csv:
...
#              Tags are normalized via the crosswalk's inat_variants.
#   5. FIELDS  pull each observation's observation fields, restricted to
#              crosswalk rows where applies_to is "plant" or "both".
#              Folded synonyms share one canonical via ;-separated field_id
#              lists. Fields marked type="ignore" are suppressed from the
#              unknown report. Unknown fields are reported for crosswalk review.
#   6. COUNTS  update plant_count in the crosswalk for any obs_field/
#              notes_field rows where applies_to is "plant" or "both".
...
#
# Outputs:
#   data/outputs/inat_clean/cabr_inat_plant_clean.csv     -- cleaned + triaged plant obs
#   data/outputs/inat_clean/qc/cabr_inat_plant_unknown_fields.csv      -- fields seen but not in crosswalk
#   data/outputs/inat_clean/qc/cabr_inat_plant_unknown_tags.csv        -- tags seen but not in crosswalk
# =============================================================
...
TAXON_PLANTS <- 47126   # Plantae
UA           <- "beescabr inat_plant_clean (brandirenesanchez16@gmail.com)"
...
if (all(c("datatype", "allowed_values") %in% names(crosswalk))) {
  ...
  if (length(need) > 0) {
    ...
    for (i in need) {
      pid <- trimws(strsplit(crosswalk$field_id[i], ";")[[1]][1])
      def <- tryCatch(
...
obs_tags <- tagged |>
  ...
  summarise(
    ...
    transect    = { tr <- canonical[category == "Transect"]
                    tr <- if_else(str_starts(tr, "TP"), "TP", tr)
...
# ---- 4. Field extraction (crosswalk-driven, plant + both only) -------------
# Only extract fields that apply to plant observations. Bee-only fields
# (applies_to == "bee") are intentionally excluded from the plant output.
field_rows <- crosswalk |>
  filter(type %in% c("obs_field", "notes_field"),
         applies_to %in% c("plant", "both"))
...
ofv_mapped <- ofv_long |>
  left_join(id_map, by = "field_id") |>
  mutate(key = norm_key(field_name)) |>
  left_join(name_map, by = "key", suffix = c("", "_nm")) |>
  mutate(canonical = coalesce(canonical, canonical_nm)) |>
  select(-canonical_nm, -key)

# 4a. Fields NOT in the crosswalk -> discovery report
...
clean <- obs_base |>
  ...
  relocate(obs_id, observer, data_source, observed_date, scientific_name, common_name,
           triage, triage_reason, survey_year, survey_type, transect, is_10min)

# ---- 6. Update plant_count in crosswalk ------------------------------------
plant_field_counts <- ofv_mapped |>
  filter(!is.na(canonical)) |>
  group_by(canonical) |>
  summarise(plant_count = n_distinct(obs_id), .groups = "drop")

crosswalk_updated <- crosswalk
for (i in seq_len(nrow(plant_field_counts))) {
  nm  <- plant_field_counts$canonical[i]
  cnt <- plant_field_counts$plant_count[i]
  row <- which(crosswalk_updated$name == nm)
  if (length(row) == 1)
    crosswalk_updated$plant_count[row] <- as.character(cnt)
}

write_csv(crosswalk_updated, crosswalk_path, na = "")
message("Updated plant_count in crosswalk -> ", crosswalk_path)

# ---- 7. Write + summary -----------------------------------------------------
if (nrow(clean) == 0) {
  cat("No plant observations returned for the current scope.\n")
} else {
  write_fresh(clean, out_clean)
  cat("\n--- PLANT CLEANING SUMMARY ---\n")
  ...
  if (nf > 0 || nt > 0) {
    ...
    if (nf > 0)
      cat(sprintf("* %d new FIELD(S) not in the crosswalk.\n", nf),
          ...
          "       If keeping, add a row to master_crosswalk.csv with applies_to = plant or both.\n", sep = "")
    if (nt > 0)
    ...
    if (interactive()) {
      answer <- readline("Have you reviewed and updated master_crosswalk.csv? (y/n): ")
      if (tolower(answer) == "y") {
        cat("Re-running triage with updated crosswalk...\n")
        source("scripts/clean/inat_plant_clean.R")
      } else {
        stop("Stopped. Update master_crosswalk.csv first, then re-run this script.")
      }
    }
  } else {
