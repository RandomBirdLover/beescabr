# =============================================================
# clean/finding_survey_dates.R
# beescabr -- builds the per-survey record (per_survey_information.csv): one row per
...
# FPI_SURVEY_DATES, FPI_SPECIMENS, FPI_REVIEW, SD_WINDOW_TOL_DAYS); it is sourced BY the
...
# ------------------------------------------------------------
# per_survey_information.csv  +  review_beeple_survey_windows.csv  (tag-first rewrite 2026-07-17)
# Survey dates for BOTH methods (lethal net + non-lethal iNaturalist) and BOTH roles
# (intern + beeple):
#   * INTERNS (lethal net AND non-lethal iNat) -> PRESERVED as-is from per_survey_information.csv
...
# per_survey_information.csv = CONFIRMED surveys only. Returns list(survey_dates, review).
# ------------------------------------------------------------
...
fpi_survey_dates <- function(membership, windows, roster,
                             ...
                             spec_path = FPI_SPECIMENS, tol_days = SD_WINDOW_TOL_DAYS) {
  blank <- function(x) is.na(x) | trimws(as.character(x)) == ""

  # ---- INTERNS: preserved as-is from per_survey_information.csv (people edit them there) ----
  ...
  # They live IN per_survey_information.csv as the source=="intern-log" rows -- BOTH lethal net days
  ...
  # them. Add / fix intern surveys by editing per_survey_information.csv.
  interns <- tibble()
  ...
  # specimen_dates.csv (date, n_specimens, collectors) is the aggregated specimen record. It
  ...
  # unlike intern-log rows they are not preserved -- edit specimen_dates.csv, never these rows.
  spec_lu <- tibble(date = as.Date(character()), n_speci = integer(), collectors = character())
  if (file.exists(spec_path)) {
    sp <- suppressWarnings(read_csv(spec_path, show_col_types = FALSE))
    if (all(c("date", "n_specimens") %in% names(sp)))
  ...
  # are already preserved from per_survey_information.csv above, so regenerating them from tags would
