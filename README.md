...
Includes the bee field guides (species + genus), park summary tables, least-sampled bees, and the survey-transect and bee-bounty maps. The pages are generated by the pipeline and published with `bash scripts/publish_pages.sh` (copies the public HTML into `docs/`, which GitHub Pages serves).
