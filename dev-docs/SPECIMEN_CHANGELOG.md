...
Tracks changes to `cabr_bee_specimens_record_V{n}_{YYYY_MM_DD}.xlsx` in `data/specimens/records/`.

---

## How versioning works

All versions of the specimen file live together in `data/specimens/records/`. The R pipeline always uses the newest one automatically — no manual archiving needed.

**When you update the specimen file:**
1. Open the current version in Excel
2. Make your changes
3. Save As → `cabr_bee_specimens_record_V{n+1}_{YYYY_MM_DD}.xlsx` in the same `records/` folder
...
**Naming convention:** `cabr_bee_specimens_record_V{n}_{YYYY_MM_DD}.xlsx` where the date is when that version was finalized.
...
| V8 | 2026-06-08 | Brandi Sanchez | 1,437 | 34 | Added `Missing Specimen` column. Renamed to `cabr_bee_specimens_record_V8_2026-06-13.xlsx` on 2026-06-13 (first version with formal date attached; rows sorted by UCSD_ID ascending). |
