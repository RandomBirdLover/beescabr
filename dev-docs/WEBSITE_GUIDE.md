...
**To see them,** open `bee_themes_pallete.png` in this folder. It is a rendered swatch of
every group, regenerated by `scripts/analysis/render_palette.R` on each pipeline run, so
it never drifts from the code.

**There are currently no hex values outside the theme.** That is the point of the rule, and
it is worth knowing what it fixed.

Pages used to be written with colours typed directly into the script, like
`color:#1e5a2b` sitting in `bee_field_guide.R`. Copies like that drift. Three different
deep greens (`#1c5728`, `#1e5a2b`, `#1c5c28`) were in use across pages that were all meant
to match, and nothing ever failed, because near-identical greens look fine side by side.
They only diverge visibly when somebody changes the palette and half the site moves while
the other half does not.

All 37 of those values were routed through the theme. Doing so changed exactly one thing
on the live site: the field guides' headings, which had been on the drifted green and are
now the same one as everywhere else.
